...
export function parseContactNo(contactNo?: string | null) {
  const normalized = contactNo?.trim() ?? "";
  if (!normalized) return { countryCode: "+92", phoneNumber: "" };

  const compactContactNo = normalized.replace(/[\s()-]/g, "");
  const match = countryCallingCodes
    .slice()
    .sort((a, b) => b.code.length - a.code.length)
    .find((entry) => compactContactNo.startsWith(entry.code.replace(/-/g, "")));

  if (!match) return { countryCode: "+92", phoneNumber: normalized };

  const compactCode = match.code.replace(/-/g, "");
  const phoneNumber = compactContactNo.slice(compactCode.length);
  return { countryCode: match.code, phoneNumber };
}

export function formatContactNo(countryCode: string, phoneNumber: string) {
  const cleanedNumber = phoneNumber.replace(/[^\d]/g, "");
  if (!cleanedNumber) return "";
  return `${countryCode.replace(/-/g, "")}${cleanedNumber}`;
}
